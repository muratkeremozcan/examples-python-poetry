...
def test_visualize_versus_price_input_validation(sample_visualization_data):
    ...
    with pytest.raises(KeyError):
        visualize_versus_price(sample_visualization_data[["processor_speed"]], "processor_speed")
    with pytest.raises(KeyError):
...
def test_visualize_versus_price_titles(mock_show, sample_visualization_data):
    ...
    with patch('matplotlib.pyplot.xlabel') as mock_xlabel, \
        ...
        mock_title.assert_called_once_with("Battery Capacity vs. Price", pad=20, fontsize=14, fontweight='bold')
