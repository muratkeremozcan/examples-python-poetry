...
print(f"UTC hours: {(end.astimezone(timezone.utc) - start.astimezone(timezone.utc)).total_seconds()/3600}")  # 5 hours
...
for trip in onebike_datetimes:
    ...
    if start > end:
        end = tz.enfold(end, fold=1) # ensures that if an end time appears to be before its start due to DST, it's interpreted as the later occurrence
